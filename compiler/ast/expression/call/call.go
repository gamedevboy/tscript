...
func (impl *Component) Compile(f interface{}, r *compiler.Operand) *compiler.Operand {
    ...
    if impl.isNew {
        _func.AddInstructionABC(opcode.NewCall, opcode.Flow, rf, compiler.NewSmallIntOperand(regCount),
            compiler.NewSmallIntOperand(int16(argList.Len())))
    } else {
        _func.AddInstructionABC(opcode.Call, opcode.Flow, rf, compiler.NewSmallIntOperand(regCount),
            compiler.NewSmallIntOperand(int16(argList.Len())))
    }
...
func NewCall(owner, e, argList interface{}) *Component {
    return &Component{
